...
export default {
    ...
    //获取密码强度和提示信息
    getPasswordType (pwd) {
        var strength = 0, msg = '', matchLen = 0;
        var lower = pwd.match(/[a-z]/g)
            , upper = pwd.match(/[A-Z]/g)
            , number = pwd.match(/\d/g)
            , symbol = pwd.match(/[!-/:-@\[-`{-~]/g); //英文符号
    ...
    isValidPic (file, opts = {}) {
        ...
        if (!isImg) {
            ...
            $x.toast.error($x.msg.VALIDATE.UPLOADTYPE.replace('{format}', format));
        }
        if (!isLt2M) {
            $x.toast.error($x.msg.VALIDATE.UPLOADSIZE.replace('{sizeM}', sizeM));
        }
